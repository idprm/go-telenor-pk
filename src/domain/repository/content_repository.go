package repository

import (
	"database/sql"

	"github.com/idprm/go-telenor-pk/src/domain/entity"
)

const (
	querySelectContent = "SELECT value FROM contents WHERE service_id = $1 AND name = $2 LIMIT 1"
)

type ContentRepository struct {
	db *sql.DB
}

type IContentRepository interface {
	Get(int, string) (*entity.Content, error)
}

func NewContentRepository(db *sql.DB) *ContentRepository {
	return &ContentRepository{
		db: db,
	}
}

func (r *ContentRepository) Get(serviceId int, name string) (*entity.Content, error) {
	var content entity.Content
	err := r.db.QueryRow(querySelectContent, serviceId, name).Scan(&content.Value)
	if err != nil {
		return &content, err
	}
	return &content, nil
}
